...
class App:
    ...
    def select_artist(self, artist_name: str) -> None:
        artists = self._client.search(artist_name, type="artist").page(1)

        for i in range(min(len(artists), MAX_PROPOSED_LEN)):
            print(f"[{i}] {artists[i].name}")

        current_artist: discogs_client.Artist = artists[max(0, min(MAX_PROPOSED_LEN - 1, int(input('select artist: '))))]
        ...
        aliases = [current_artist.name] + current_artist.name_variations
